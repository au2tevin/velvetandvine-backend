...
app.patch('/site/data', async (req, res) => {
  try {
    ...
    const ALLOWED = ['backgrounds','gallery','svcPhotos','promoVideo','typography','products','orders','layout'];
...
// ── START ─────────────────────────────────────────────────────
app.listen(PORT, () => {
  console.log(`🚀 Velvet & Vine backend running on port ${PORT}`);
  // Connect to MongoDB immediately on startup so errors show in logs
  connectDB()
